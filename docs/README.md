# プロジェクト ドキュメント インデックス

秋川流域花火大会 公式サイト の運用・開発ドキュメント集。

## 👤 運営チーム向け（運用・編集）

| ファイル | 用途 | 想定読者 |
|---|---|---|
| [EDITING_GUIDE.md](./EDITING_GUIDE.md) | microCMS でお知らせ・体験談・スポンサーを編集する方法 | 運営編集担当 |
| [RELEASE_RUNBOOK.md](./RELEASE_RUNBOOK.md) | サイト更新・デプロイ・ロールバック手順 | 運営 / Web 担当 |
| [INCIDENT_RUNBOOK.md](./INCIDENT_RUNBOOK.md) | 障害時の対応フロー（P0-P3） | 運営 / Web 担当 |
| [QA_CHECKLIST.md](./QA_CHECKLIST.md) | 公開前 / 直前期 / 当日のチェック項目 | 運営 / QA 担当 |

## 🎯 戦略・計画

| ファイル | 用途 |
|---|---|
| [WEB_STRATEGY_2026.md](./WEB_STRATEGY_2026.md) | 第8回 Web 戦略（来場8,000人を目指す KPI 設計） |
| [BACKLOG.md](./BACKLOG.md) | タスクバックログ（優先度別） |

## 🔌 外部サービス連携

| ファイル | 用途 |
|---|---|
| [LOOKER_STUDIO_SETUP.md](./LOOKER_STUDIO_SETUP.md) | Looker Studio で GA4 ダッシュボードを構築する手順 |
| [MONTHLY_REPORT_TEMPLATE.md](./MONTHLY_REPORT_TEMPLATE.md) | 月次 Web レポートのフォーマット（運営会議用） |
| [GA4_FUNNEL_REPORT.md](./GA4_FUNNEL_REPORT.md) | GA4 ファネル可視化（Apps Script で Spreadsheet 出力） |
| [NEXT_EDITION_SIGNUP.md](./NEXT_EDITION_SIGNUP.md) | 次回開催情報メール登録の Google フォーム連携 |

---

## クイックリンク

- **本番サイト**: https://akigawa-hanabi.pages.dev/
- **GitHub**: https://github.com/yusukehagiwara1/akigawa-hanabi
- **Cloudflare Pages**: https://dash.cloudflare.com/
- **GA4**: https://analytics.google.com/
- **Search Console**: https://search.google.com/search-console
- **microCMS**: https://0k3w9bd30b.microcms.io/
- **Looker Studio ダッシュボード**: （初回セットアップ後にここに URL を記載）
- **公式 Instagram**: https://www.instagram.com/akigawa_hanabitaikai/
